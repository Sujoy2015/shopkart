...
//        loginEntity.setImage(new byte (jsonObject.get("image")));
   ...
   @Override
    ...
    public String login(String payload) {
        ...
        String email= jsonObject.get("userEmail").toString();
        String password = jsonObject.get("userPassword").toString();

        LoginEntity loginEntity = loginrepo.checkLogin(email,password);
        if (loginEntity != null && loginEntity.getUserEmail() != null){
    ...
    @Override
    public String logout(String payload) {
        JSONObject jsonObject = new JSONObject(payload);

        String email= jsonObject.get("userEmail").toString();
        String password = jsonObject.get("userPassword").toString();

        LoginEntity loginEntity = loginrepo.checkLogout(email,password);
        if (loginEntity != null && loginEntity.getUserEmail() != null){
            return "successfully Login";
        }
        else{
